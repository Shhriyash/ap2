...
if (-not $NoClean) {
    Write-Host "Cleaning previous stack state (PIDs + occupied ports)..." -ForegroundColor DarkCyan
...
$gatewayArgs = @("-NoExit", "-ExecutionPolicy", "Bypass", "-File", $gatewayScript)
if ($Reload) {
    $gatewayArgs += "-Reload"
}
...
$agentArgs = @("-NoExit", "-ExecutionPolicy", "Bypass", "-File", $agentScript)
if ($Reload) {
    $agentArgs += "-Reload"
}
