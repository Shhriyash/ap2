...
from shared_lib.contracts.user import UserIdentityResponse, UserPinLoginRequest, UserPinVerifyRequest, UserPinVerifyResponse


class AuthService:
    async def cli_login(self, email: str, pin: str) -> SessionPrincipal:
        user = await self._verify_pin_login(email=email, pin=pin)
    ...
    async def _verify_pin_login(self, email: str, pin: str) -> UserIdentityResponse:
        payload = UserPinLoginRequest(email=email, pin=pin)
        ...
        async with httpx.AsyncClient(timeout=settings.gateway_timeout_seconds) as client:
            resp = await client.post(
                f"{settings.gateway_base_url}/users/login-pin",
                headers=headers,
                json=payload.model_dump(mode="json"),
            )
            if resp.status_code in {400, 401, 404}:
                raise PermissionError("Login rejected: invalid email or PIN.")
